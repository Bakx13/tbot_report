...
class Localization:
    def __init__(self, language: str, *, fallback: str, replacements: Dict[str, str] = None):
        self.local_path = "tbot_report.localization.locale"
        log.debug(f"Creating localization for {language}")
        self.language: str = language
        log.debug(f"{self.local_path}.{language}")
        self.module: types.ModuleType = importlib.import_module(f"{self.local_path}.{language}")
        if language != fallback:
            log.debug(f"Importing {self.local_path}.{fallback} as fallback")
            self.fallback_language: str = fallback
            self.fallback_module = importlib.import_module(f"{self.local_path}.{fallback}") if fallback else None
        else:
